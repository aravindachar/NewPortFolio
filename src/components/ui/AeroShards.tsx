...
const resolveFrameInterval = (frameState: any, refreshInterval: number) =>
...
const advanceFormation = (state: any, flow: number, elapsed: number, duration: number, frozen: boolean) => {
  ...
  if (state.weights.every((value: number, i: number) => Math.abs(value - goal[i]) < 0.0001 && Math.abs(state.velocity[i]) < 0.001)) {
    state.weights = goal;
    state.velocity.fill(0);
  }
};
...
const advanceHold = (hold: any, elapsed: number, disabled: boolean) => {
  if (disabled) {
    hold.pointerId = null;
    hold.elapsed = 0;
    hold.amount = 0;
...
const parseColor = (value?: string, fallback = '#120F17') => {
...
const resolveDpr = (preset: any, canvas: HTMLCanvasElement) => {
  const cssPixels = Math.max(1, canvas.clientWidth * canvas.clientHeight);
...
const resolveBloomSize = (size: any, qualityLevel = 0) => {
...
const createRenderGraph = (gpu: any, outputSize: any, bloomSize = resolveBloomSize(outputSize)) => {
...
const configureStyle = (graph: any, settings: any, outputSize: any, cssSize: any) => {
  ...
  graph.styleTarget.resize((mode ? outputSize : [1, 1]) as any);
...
export default function AeroShards({
  ...
  useEffect(() => {
    ...
    let timeoutId = 0;
    let unsubscribeResize: any;
    let unsubscribeGpuError: any;
    let visibilityObserver: any;
    ...
    void (async () => {
      try {
        ...
        const outputFormat = (navigator as any).gpu?.getPreferredCanvasFormat?.() || 'bgra8unorm';
        ...
        const resizePostTargets = (qualityLevel = appliedBloomLevel) => {
          ...
          graph.sceneTarget.resize([width, height] as any);
          graph.bloomTarget.resize(bloomSize as any);
          graph.bloomScratchTarget.resize(bloomSize as any);
        ...
        const resizeOutput = () => {
          ...
          if (nextSize[0] !== output.size[0] || nextSize[1] !== output.size[1]) output.resize(nextSize as any);
          resizePostTargets();
        };
