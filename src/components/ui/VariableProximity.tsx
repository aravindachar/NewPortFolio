...
function useMousePositionRef(containerRef: React.RefObject<HTMLDivElement | HTMLElement | null>) {
...
export const VariableProximity = forwardRef<HTMLSpanElement, VariableProximityProps>((props, ref) => {
  ...
  useEffect(() => {
    if (localRef.current) {
      letterElementsRef.current = Array.from(
        localRef.current.querySelectorAll<HTMLSpanElement>('.vp-letter')
      );
    }
  }, [label]);

  useAnimationFrame(() => {
    if (!containerRef?.current || letterElementsRef.current.length === 0) return;
    ...
    letterElementsRef.current.forEach(letterEl => {
      if (!letterEl) return;
